window.onload = function (){	
	...
	var camera = {
		...
		setTarget: function (elem) {
			...
			// set target
			if (this.targeted) {
				this.targeted.style.outline = "rgba(0,255,0,1) solid 10px";
			}
			if (this.targeted != elem) {
				...
				elem.style.outline = "rgba(0,0,255,1) solid 10px";
				this.targeted = elem;
			} else {
				this.z.setTarget(0);
				if (elem.id == "7") {
	...
	run();
	(function(){		
		var Link = document.getElementsByClassName("click");
		for (var i=0;i<Link.length;i++) {
			Link[i].ondblclick = function(){
				window.open(this.getAttribute("_src"))
			};
		};
	})();	
